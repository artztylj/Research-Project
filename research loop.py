...
for i in range(len(seeds)):
    ...
    for ratio in ratios:
        ...
        df.rename(columns={"Open": f"{ratio}_open", "High": f"{ratio}_high", "Low": f"{ratio}_low",
    ...
    glorot_initializer = tf.keras.initializers.glorot_uniform(seed=seeds[i])
    orthogonal_initializer = tf.keras.initializers.Orthogonal(gain=1.0, seed=seeds[i])
